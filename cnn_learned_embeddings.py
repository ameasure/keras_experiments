...
raw_train, raw_test = get_data(n_train=20000, n_test=10000)
...
filter_lengths = [800, 600, 400, 300, 200]
conv_filters = []
...
for n, n_gram in enumerate(ngram_filters):
    ...
    sequential.add(Convolution2D(nb_filter=n_filters, stack_size=1, 
...
fan_in = n_filters * len(conv_filters)
model.add(Dense(fan_in, fan_in))
model.add(Activation("relu"))
model.add(BatchNormalization(fan_in))
model.add(Dropout(0.5))
...
history = model.fit(X_train_concat, Y_train, nb_epoch=10, batch_size=batch_size, 
