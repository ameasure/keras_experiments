...
for n, n_gram in enumerate(ngram_filters):
    ...
    # the output of this layer is going to be rows=max_len - n_gram + 1, by 1 column
    sequential.add(Convolution2D(nb_filter=400, stack_size=filter_lengths[n], 
                                 nb_row=2, nb_col=1))
    sequential.add(Activation("relu"))
    sequential.add(MaxPooling2D(poolsize=(max_len - n_gram + 1 - 2 + 1, 1)))
